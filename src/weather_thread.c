...
void fetch_weather_data_multithreaded(GeoLoc *geoArray,
  ...
  for (int i = 0; i < THREAD_COUNT; i++) {
    ...
    if (pthread_create(&threads[i], NULL, thread_worker, args) != 0) {
      ...
      log_error("Error in creating fetching data thread");
      sem_destroy(&args->semaphore);
      free(args);
    }
    ...
    if (pthread_create(&send_threads[i], NULL, send_worker, args) != 0) {
      ...
      log_error("Error in creating sending data thread");
      sem_destroy(&args->semaphore);
      free(args);
    }
  }
  ...
  log_info("Client is deleted.\n");
  log_info("Weather data fetching and sending completed.\n");
}