...
char* build_url(const char* base_url, const char* lat, const char* lon, const char* units, const char* api_key) {
    ...
    snprintf(url, len, "https://api.openweathermap.org/data/2.5/weather?lat=%s&lon=%s&units=%s&appid=%s", lat, lon, units, api_key);
    return url;
}

int main() {
    // URL API OpenWeatherMap z Twoim kluczem API (zastąp YOUR_API_KEY własnym kluczem)
    ...
    // Zmienna z kluczem API (wstaw swój klucz API)
    const char *api_key = "";


    
    // Budowanie URL na podstawie zmiennych
